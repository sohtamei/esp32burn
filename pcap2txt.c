...
void parse_esp32burn(int endp, uint8_t* buf, int size);
void finish_esp32burn(void);
int main(int argc, char* argv[])
{
	...
	if(argc <= 1) {
		printf("usage: pcap2txt filename [0-brdongle, 1-meshbridge, 2-esp32burn] \n");
		return 1;
	}
	int mode = 0;
	enum {
	...
	//	for(i=0;i<size;i++) printf("%02x",buf[i]); printf("\n");
		...
		switch(mode) {
		default:
		case MODE_BTDONGLE:		parse_btdongle(endp, buf, size);	break;
		case MODE_MESHBRIDGE:	parse_meshbridge(endp, buf, size);	break;
		case MODE_ESP32BURN:	parse_esp32burn(endp, buf, size);	break;
//		case MODE_DONT_PARSE) {
...
Error:
	switch(mode) {
//	default:
//	case MODE_BTDONGLE:			break;
//	case MODE_MESHBRIDGE:		break;
	case MODE_ESP32BURN:	finish_esp32burn();	break;
	}
	if(fp) fclose(fp);
	return result;
}
...
struct {int com; char mes[128];} const burnMes[] = {
  { 0x02, "flash_begin" },
  { 0x03, "flash_data" },
  { 0x04, "flash_end" },
  { 0x05, "mem_begin            size     blocks   blocksize offset" },
  { 0x06, "mem_end                       entrypoint" },
  { 0x07, "mem_data             data     seq" },
  { 0x08, "sync" },
  { 0x09, "write_reg            addr     value    mask     delay_us" },
  { 0x0a, "read_reg             adrs" },
  { 0x0b, "set_params           fl_id    total_size block_size sector_size page_size status_mask" },
  { 0x0d, "attach" },
  { 0x0e, "read_flash_slow" },
  { 0x0f, "change_baudrate" },
  { 0x10, "flash_defl_begin     write_size num_blocks WRITE_SIZE offset" },
  { 0x11, "flash_defl_data      len      seq" },
  { 0x12, "flash_defl_end       reboot" },
  { 0x13, "flash_md5            addr     size" },
};

int last_com = 0;
void parse_c0buf(int endp, uint8_t* buf, int size)
{
	int i;
	switch(endp) {
	case 0x01:
	case 0x02:
		if(last_com != buf[2]) {
			for(i = 0; i < sizeof(burnMes)/sizeof(burnMes[0]); i++) {
				if(burnMes[i].com == buf[2]) {
					printf("\n# %s\n", burnMes[i].mes);
					break;
				}
			}
	...
	for(i = 0; i < size-1; i++) {
		if(buf[i+0] == 0xdb) {
			if(buf[i+1] == 0xdc) {
				memmove(buf+i+0, buf+i+1, size-(i+1));
				buf[i+0] = 0xc0;
				size--;
	...
	for(i = 0; i < size;) {
		switch(i) {
		case 3:
			printf(" %02x%02x", buf[i+1], buf[i+0]);
			i+=2;
			break;
		case 5:
		case 9:
		...
		case 21:
			if(size-i >= 4) {
				printf(" %02x%02x%02x%02x", buf[i+3], buf[i+2], buf[i+1], buf[i+0]);
				i+=4;
				break;
			}
		case 25:
...
void parse_c0bufs(int endp, uint8_t* buf, int size)
{
	int i;
	int startCount = 0;
	for(i = 1; i < size - 1; i++) {
		if(buf[i-1] == 0xc0 && buf[i] == 0xc0) {
			parse_c0buf(endp, buf+startCount, i-startCount);
			startCount = i;
		}
	}
	parse_c0buf(endp, buf+startCount, size-startCount);
}
...
void parse_esp32burn(int endp, uint8_t* buf, int size)
{
	if(last_endp != endp && c0bufCount) {
		parse_c0bufs(last_endp, c0buf, c0bufCount);
		c0bufCount = 0;
	}
	last_endp = endp;
	memcpy(c0buf+c0bufCount, buf, size);
	c0bufCount += size;
}
